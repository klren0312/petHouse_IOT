...
P_Window init_btc_window()
{
    ...
    add_text_layer(p_window, &g_layer_id_data, "waiting", &frame_data, GAlignCenter , U_ASCII_ARIAL_16, GColorWhite);
...
static void web_recv_callback(const uint8_t *buff, uint16_t size)
{
    ...
    /*添加数据*/
    GRect frame_data = {{0,60}, {16, 128}};
    add_text_layer(h_window, &g_layer_id_data, stock_gid, &frame_data, GAlignCenter , U_ASCII_ARIAL_16, GColorWhite);
